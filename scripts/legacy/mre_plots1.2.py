...
def main():
    ...
    with open(input_file, "r") as fh_input:
        for sample_gene_lengths in fh_input:
            ...
            #if sample not in sample_debug_dict.keys():
            #    sample_debug_dict[sample] = 0
            #sample_debug_dict[sample] += 1
            ...
            for sample_group_ancestry_list in ancestry_table_content:
                ...
                if sample in sample_group_ancestry_list:
                    ...
                    if group_plus_ancestry not in ancestry_to_gene_to_length:
                        ancestry_to_gene_to_length[group_plus_ancestry] = {gene: lengths}
    ...
    """
    for group, gene_len_dict in ancestry_group_to_gene_to_length.items():
        if group == "NAT":
            for focad, lengths in gene_len_dict.items():
                if focad == "FOCAD1":
    ...
    # TODO debug: NAT has too high numbers
